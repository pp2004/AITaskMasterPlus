...
def _new_fragment_id_queue(
    ...
    if scope == "app":
        ...
        #     triggered by st.experimental_rerun()) because it defers the execution of st.experimental_rerun().
...
def rerun(  # type: ignore[misc]
    *,  # The scope argument can only be passed via keyword.
    ...
    When ``st.experimental_rerun()`` is called, Streamlit halts the current script run and
